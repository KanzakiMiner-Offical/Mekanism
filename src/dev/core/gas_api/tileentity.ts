class TileEntityInterfaceWithGas
    implements GasStorage {
    readonly liquidUnitRatio: number = 1;
    readonly gasUnitRatio: number = 1;
    readonly slots?: {
        [key: string]: SlotData
    };
    ...
    gasStorage: BlockEngine.GasTank
    ...
    receiveGas(gasStorage: IGasStorage, gas: string, amount: number): number {
        let storedGas = gasStorage.getGasStored();
